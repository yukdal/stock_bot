...
def build_prompt(filtered_stocks):
    ...
    for idx, s in enumerate(filtered_stocks, 1):
        ...
        for n_idx, n in enumerate(s["news"], 1):
            news_str += f"   - [{n['source']}] {n['title']} (링크: {n.get('link', '')})\n"
...
[핵심 지시사항 - 반드시 엄수할 것]
1. 제공된 데이터 중 스윙 매매 관점에서 가장 상승 확률이 높은 **단 1~3개 종목만**을 '오늘의 TOP 스윙 추천주'로 엄선하십시오.
2. 엄선된 1~3개 종목에 대해서만 아래의 상세 분석 블록(■ 종목명... ~ 📰 관련 핵심 뉴스...)을 작성하며, 예상 매수가, 목표 매도가, 손절가, 추천 사유를 당신의 전문 지식을 활용해 구체적으로 작성해야 합니다.
3. **선정되지 않은 나머지 종목들(4순위 이하 전체)에 대해서는 절대로 상세 분석 블록을 생성하지 마십시오.** 이 종목들은 오직 맨 아래의 '📊 필터링 통과 종목 전체 요약' 마크다운 표에만 데이터를 기입해야 합니다. (요약 표에는 1~3순위 종목을 포함한 전체 종목을 모두 작성)

[출력 양식 가이드 시작]
🚀 오후 8시 Gemini Pro 연동 스윙 주도주 브리핑

🏆 오늘의 TOP 스윙 추천주
(※ 가장 상승 확률이 높은 1~3개 종목만 엄선하여 아래 양식으로 작성하십시오.)
■ 종목명: {{종목명}} ({{티커}}) {{시장구분}}
- 주도 테마: {{핵심 주도 테마명 1~2개}}
...
📰 관련 핵심 뉴스 (Gemini 요약)
- "[{{뉴스 데이터에서 당일 급등 원인이 된 가장 중요한 핵심 헤드라인 1개 발췌 및 요약}}]({{해당 뉴스의 링크 주소}})" (출처: {{해당 뉴스의 실제 출처}})

--------------------------------------
(※ 경고: 위 선 아래로는 절대 개별 종목의 상세 분석 블록(■ 종목명...)을 추가로 생성하지 마십시오! TOP 추천주로 선정되지 않은 나머지 종목들은 무조건 아래 요약 표에만 작성해야 합니다.)

📊 필터링 통과 종목 전체 요약
| 종목명 | 티커 | 등락률 | 거래대금 | MA1000 위치 | 외국인수급 | 기관수급 |
|---|---|---|---|---|---|---|
(여기에 제공된 1순위부터 끝까지의 '모든' 통과 종목 데이터를 마크다운 표로 작성)
[출력 양식 가이드 끝]
